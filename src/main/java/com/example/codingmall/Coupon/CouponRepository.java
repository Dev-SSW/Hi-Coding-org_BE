...
public interface CouponRepository extends JpaRepository<Coupon, Long> {
    default Coupon findCouponById(Long couponId) {
        return findById(couponId).orElseThrow(() -> new CouponNotFoundException("존재하지 않는 쿠폰 ID 입니다 : " + couponId));
    }
    ...
    default Coupon findCouponByCouponName(String name) {
        return findByCouponName(name).orElseThrow(() -> new CouponNotFoundException("쿠폰 이름에 일치하는 쿠폰이 없습니다. : " + name));
    }
}
